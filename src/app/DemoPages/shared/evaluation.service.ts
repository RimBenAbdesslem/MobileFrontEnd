...
@Injectable({
  ...
  export class EvaluationService {
    constructor(private fb: FormBuilder,private For : FormBuilder, private fbPro: FormBuilder, private F: FormBuilder,private FB: FormBuilder,private Fb: FormBuilder,private http: HttpClient,private router: Router){}
    ...
    EvaluationFroidModel = this.fb.group({
     ...
     AjouterProduit = this.fbPro.group({
      ...
      SCategorie: ['', Validators.required],
     });


     registerProduit(){
       var produit={
        Nom:this.AjouterProduit.value.prodname,
        Description:this.AjouterProduit.value.description,
        Prix:this.AjouterProduit.value.prix,
        DateDebut:this.AjouterProduit.value.dateDebut,
         DteFin:this.AjouterProduit.value.dateFin,
         NonFournisseur: this.AjouterProduit.value.fourname,
         LienFace: this.AjouterProduit.value.lien,
         Scategorie: this.AjouterProduit.value.SCategorie,
         CategorieId: 1,
         Categorie: this.AjouterProduit.value.Categorie,
         TypeId: 1,
         Quantite: this.AjouterProduit.value.quantite,
         Image:"hggj"
       }
       return this.http.post('https://localhost:44369/api/Produit/RegisterProduit',produit);
     }
